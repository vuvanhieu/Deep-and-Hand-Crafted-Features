...
model_name = 'scenario_1_Tranformer_1'
...
# Encoder for labels
train_labels_encoded = label_encoder.fit_transform(train_hog_labels)  # Use any label set, here hog labels are used for demonstration
...
scaler_histogramSeg = StandardScaler()
...
X_test_combined = np.concatenate([test_hog_features, test_lbp_features, test_ccv_features], axis=-1)
y_test_combined = test_labels_categorical  # Assuming this is your categorical test labels

# Splitting the combined test set into validation and new test sets (80% validation, 20% test)
...
val_hog_features = X_val_combined[:, :, :hog_features_dim]
val_lbp_features = X_val_combined[:, :, hog_features_dim:hog_features_dim+lbp_features_dim]
val_ccv_features = X_val_combined[:, :, -ccv_features_dim:]

test_hog_features = X_test_combined[:, :, :hog_features_dim]
test_lbp_features = X_test_combined[:, :, hog_features_dim:hog_features_dim+lbp_features_dim]
test_ccv_features = X_test_combined[:, :, -ccv_features_dim:]
...
train_ccv_features = train_features_combined_smote[:, cumulative_dim_lbp:].reshape(-1, 1, ccv_features_dim)
...
def build_transformer_model(hp):
    inputs = []
    for feature_dim in [hog_features_dim, lbp_features_dim, ccv_features_dim]:
...
tuner.search(
            ...
            validation_data=([val_hog_features, val_lbp_features, val_ccv_features], 
                 y_val_combined),
            epochs=epoch,
...
test_loss, test_accuracy = model.evaluate(
    [test_hog_features, test_lbp_features, test_ccv_features], 
    y_test_combined
)


# y_pred model predictions
y_pred = model.predict([test_hog_features, test_lbp_features, test_ccv_features])
